...
k_timer service_handle_timer;
k_timer parking_message_timer;
k_timer handle_uart_event_timer;
k_timer update_number_device_timer;
k_timer report_network_state_timer;

extern uint8_t IsInitApp;
// extern extern	k_msgq		queue_data_tx;
...
CHIP_ERROR AppTaskCommon::StartApp(void)
{
    ...
    // chip::Server::GetInstance().ScheduleFactoryReset();
    AppEvent event = {};
    k_timer_init(&service_handle_timer, [](k_timer * timer) {
        AppEvent event;
        event.Type    = AppEvent::kEventType_ServiceHandler;
        event.Handler = [](AppEvent *) { service_handler(); };
        GetAppTask().PostEvent(&event);
    }, nullptr);
    k_timer_start(&service_handle_timer, K_MSEC(10), K_MSEC(10));

    k_timer_init(&parking_message_timer, [](k_timer * timer) {
        AppEvent event;
        event.Type    = AppEvent::kEventType_ParkingMessage;
    ...
    k_timer_init(&handle_uart_event_timer, [](k_timer * timer) {
        AppEvent event;
        event.Type    = AppEvent::kEventType_HandleUartEvent;
    ...
    if (!IsInitApp) {
        k_timer_init(&update_number_device_timer, [](k_timer * timer) {
            AppEvent event;
            event.Type    = AppEvent::kEventType_UpdateNumberDevice;
            event.Handler = [](AppEvent *) { GetAppTask().UpdateNumberDevice(); };
            GetAppTask().PostEvent(&event);
        }, nullptr);
        k_timer_start(&update_number_device_timer, K_MSEC(1000), K_MSEC(1000));
    }

    // k_timer_init(&report_network_state_timer, [](k_timer * timer) {
    //     AppEvent event;
    //     event.Type    = AppEvent::kEventType_ReportNetworkStatus;
    //     event.Handler = [](AppEvent *) { GetAppTask().ReportNetworkState(); };
    //     GetAppTask().PostEvent(&event);
    // }, nullptr);
    // k_timer_start(&report_network_state_timer, K_MSEC(10000), K_MSEC(10000));


    // report status network
    uint8_t network_status = 0x03;
    GetAppTask().SetNetworkState(network_status);
    // query product info
...
void AppTaskCommon::StartThreadHandler(AppEvent * aEvent)
{
    ...
    if (!chip::DeviceLayer::ConnectivityMgr().IsThreadProvisioned())
    {
        ...
        GetAppTask().SetNetworkState(0x01);
    }
    else
    {
        LOG_INF("Device already commissioned");
